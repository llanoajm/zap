#!/usr/bin/env bash
# Autonomous improve loop: repeatedly run a headless Claude agent against
# IMPROVE_PROMPT.md to build/train/iterate the JEPA x zap OPF surrogate-planner.
...
PROMPT="$BUILD_DIR/IMPROVE_PROMPT.md"
DONE_FILE="$BUILD_DIR/state/IMPROVE_STOP_NEVER"
...
echo "=== improve loop started $(date -u +%FT%TZ) | budget=${BUDGET_HOURS}h max_iters=$MAX_ITERS ===" >>"$LOG_DIR/loop.log"
...
for i in $(seq 1 "$MAX_ITERS"); do
    ...
    git diff --cached --quiet || git commit -m "improve: iter $i autosave" >>"$LOG_DIR/loop.log" 2>&1
    push_with_retry
...
if [[ -f "$DONE_FILE" ]]; then
    echo "=== improve loop finished: ALL_DONE ===" >>"$LOG_DIR/loop.log"
else
    echo "=== improve loop finished: budget/max-iters/abort ===" >>"$LOG_DIR/loop.log"
fi
git add ralph/build >/dev/null 2>&1
git diff --cached --quiet || git commit -m "improve: final autosave" >>"$LOG_DIR/loop.log" 2>&1
push_with_retry
