"""
Improvement experiment v10: Deeper GNN (4 MP layers) for better congestion modeling.

v9 has connecticut_16h as a persistent LMP outlier (LMP=4.75) due to network congestion.
Congestion is a global phenomenon requiring information propagation across the full network.
With 3 MP layers, the effective receptive field is nodes within 3 hops — may miss
global congestion patterns in large networks (connecticut: 156 buses, 281 branches).

Changes vs v9:
- n_mp_layers: 3 → 4 (wider receptive field)
- hidden_dim: 64 → 96 (more capacity to represent congested states)
- Everything else same: dc_frac decoder, dc_va node feat, dc_flow_frac edge feat, ranking loss

Metrics: state/improve_v10_metrics.json
Checkpoint: state/checkpoints/design_a_v10.pt
"""
...
METRICS_OUT = "ralph/build/state/improve_v10_metrics.json"
CKPT_PATH   = "ralph/build/state/checkpoints/design_a_v10.pt"
...
class DesignAv10(nn.Module):
    def __init__(self, hidden_dim=96, n_mp_layers=4):
        super().__init__()
        D = hidden_dim
...
def train():
    ...
    print(f"Train good: {len(train_good)}/{len(train_samples)}")
    ...
    model = DesignAv10(hidden_dim=HIDDEN_DIM, n_mp_layers=N_LAYERS)
    ...
    metrics = {
        "experiment": "improve_v10_deeper",
        "changes": ["n_mp_layers 3→4, hidden_dim 64→96",
                    "all v9 features: dc_frac+dc_va+dc_flow_frac+ranking"],
    ...
    print(f"  v10 (deeper): cost_gap={m8['cost_gap']['mean']:.4f}  lmp_mae={m8['lmp_mae']['mean']:.4f}")
