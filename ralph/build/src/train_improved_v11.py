"""
Improvement experiment v11: Higher aux cost weight (2.0 vs 0.5).

Root cause of connecticut_16h LMP error (MAE=4.75):
- Generators with dc_frac=0.1 (true cost 7.7+) are predicted with cost 0.77-4.99
- Model learned: dc_frac=0.1 → low-cost generator (from training states)
- In connecticut, dc_frac=0.1 means "not dispatched in DC reference" (which is expensive)
- LP dispatches these cheap-predicted-but-expensive-true generators → wrong marginal → wrong LMP

Fix: increase W_AUX_COST from 0.5 → 2.0 to force the decoder to predict individual
generator costs more accurately, regardless of their dc_frac signal.

Uses v9 architecture (all features: dc_frac decoder + dc_va node + dc_flow_frac edge + ranking).

Metrics: state/improve_v11_metrics.json
Checkpoint: state/checkpoints/design_a_v11.pt
"""
...
METRICS_OUT = "ralph/build/state/improve_v11_metrics.json"
CKPT_PATH   = "ralph/build/state/checkpoints/design_a_v11.pt"
...
W_DISPATCH  = 1.0
W_COST      = 0.1
W_AUX_COST  = 2.0   # key change: 0.5 → 2.0
W_RANK      = 0.2
RANK_MARGIN = 0.05
...
def train():
    ...
    print(f"Train: {len(train_good)}/{len(train_samples)}  filtered: {train_bad}")
    ...
    metrics = {
        "experiment": "improve_v11_higher_aux",
        "changes": ["W_AUX_COST 0.5→2.0 (4× higher cost supervision)",
    ...
    print("\n=== Summary (orig 8 test) ===")
    print(f"  v5: cost_gap=0.1235  lmp_mae=0.9993")
    print(f"  v7: cost_gap=0.1056  lmp_mae=0.9641")
    print(f"  v8: cost_gap=0.0469  lmp_mae=1.1537")
    print(f"  v9: cost_gap=0.0818  lmp_mae=0.8227")
    print(f"  v11: cost_gap={m8['cost_gap']['mean']:.4f}  lmp_mae={m8['lmp_mae']['mean']:.4f}")
