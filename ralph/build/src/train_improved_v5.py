"""
Improvement experiment v5: GridSFM DC dispatch fraction as decoder feature.

Key change: the cost decoder receives [h_gen || gen_dc_frac] where gen_dc_frac
is the GridSFM DC reference dispatch fraction (pg_ref / pmax). This fraction has
Spearman rank correlation -0.93 to -0.98 with true gen costs across all tested
states — it directly encodes the merit order.

Why this helps:
- v2 (no gen features) Spearman rank corr ≈ 0 for test states (kansas, oregon)
- DC dispatch fraction has -0.93+ corr for ALL states (train and test)
- Cheap generators run near full capacity; expensive ones run minimally or not at all
- This feature is available for ALL GridSFM states (train + test) from dc_results.json

vs v3 (pmax features): pmax→cost correlation is state-specific; DC dispatch fraction
directly reflects the actual dispatch economics and generalizes across states.

Metrics: state/improve_v5_metrics.json
Checkpoint: state/checkpoints/design_a_v5.pt
"""
...
METRICS_OUT = "ralph/build/state/improve_v5_metrics.json"
CKPT_PATH   = "ralph/build/state/checkpoints/design_a_v5.pt"
...
W_DISPATCH  = 1.0
W_COST      = 0.1
W_AUX_COST  = 0.5   # same as v2 (best for cost_gap)
LMP_MAX_THRESHOLD = 100.0


# ---------------------------------------------------------------------------
# DesignA v5: cost decoder uses h_gen + dc_dispatch_frac
# ---------------------------------------------------------------------------

class DesignAv5(nn.Module):
    """
    GNN encoder + DC-dispatch-aware cost decoder.

    The cost decoder takes [h_gen || gen_dc_frac] — the GridSFM DC reference
    dispatch fraction per generator. This directly encodes the merit order
    (Spearman rank corr ~-0.95 with true costs) and is available for test
    states from their dc_results.json files.

    Key advantage over pmax features (v3): DC dispatch fraction correlates
    with costs across ALL states (test and train), not just training states.
    """
    ...
    def forward(
        ...
        gen_dc_frac: torch.Tensor,   # (n_gen,) DC dispatch fraction
    ) -> torch.Tensor:
        h = self.encoder(node_feats, edge_index, edge_feats)   # (n_bus, D)
        h_gen = h[gen_bus]                                      # (n_gen, D)
        # Concatenate DC dispatch fraction as a conditioning feature
        dc_frac_feat = gen_dc_frac.unsqueeze(-1)               # (n_gen, 1)
...
def train():
    ...
    # Check DC frac quality
    from scipy.stats import spearmanr
    sample_corrs = []
    for s in train_good[:5]:
        if s.gen_dc_frac is not None and len(s.gen_dc_frac) > 2:
            corr, _ = spearmanr(s.gen_dc_frac, s.gen_cost)
            sample_corrs.append(corr)
    if sample_corrs:
        print(f"DC frac Spearman corr with true cost (first 5 samples): {[f'{c:.3f}' for c in sample_corrs]}")
    ...
    metrics = {
        "experiment": "improve_v5_dc_frac",
        "changes": [
            "decoder input: h_gen + gen_dc_frac (GridSFM DC reference dispatch fraction)",
            "DC dispatch fraction has Spearman rank corr ~-0.95 with true gen costs",
            "W_AUX=0.5 (same as best v2), 120 epochs",
        ],
    ...
    print(f"  v5 (dc_frac): cost_gap={m8['cost_gap']['mean']:.4f}  lmp_mae={m8['lmp_mae']['mean']:.4f}")
