"""
Improvement experiment v7: Pairwise ranking loss on costs + 150 epochs from scratch.

Key changes vs v5:
1. Add pairwise ranking loss on predicted vs true generator costs.
   - LMP accuracy depends on getting the marginal generator right (merit order).
   - LP duals are piecewise-constant in cost → LMP gradients are 0 for direct supervision.
   - Ranking loss explicitly penalizes merit-order violations: pairs (i,j) where
     true_cost[i] < true_cost[j] but pred_cost[i] >= pred_cost[j].
   - Uses soft margin: max(0, pred_cost[i] - pred_cost[j] + margin)
2. 150 epochs (v5 converged at 120; v6 fine-tune overfit → sweet spot is ~150)

Hypothesis: correct merit order → correct marginal generator → correct LMPs.
This should reduce the connecticut_16h LMP outlier (5.02 → much lower).

Metrics: state/improve_v7_metrics.json
Checkpoint: state/checkpoints/design_a_v7.pt
"""
...
METRICS_OUT = "ralph/build/state/improve_v7_metrics.json"
CKPT_PATH   = "ralph/build/state/checkpoints/design_a_v7.pt"
...
W_COST      = 0.1
W_AUX_COST  = 0.5
W_RANK      = 0.2     # pairwise ranking loss weight
RANK_MARGIN = 0.05    # soft margin for ranking loss
...
def pairwise_rank_loss(pred_cost: torch.Tensor, true_cost: torch.Tensor, margin: float = 0.05) -> torch.Tensor:
    """
    Soft pairwise ranking loss.
    For each pair (i,j) where true_cost[i] < true_cost[j]:
      penalize max(0, pred_cost[i] - pred_cost[j] + margin).
    Encourages pred_cost[i] < pred_cost[j] - margin (correct merit order with margin).
    """
    n = pred_cost.shape[0]
    if n <= 1:
        return torch.tensor(0.0)
    # Build diff matrices: true_diff[i,j] = true[i] - true[j], pred_diff[i,j] = pred[i] - pred[j]
    tc = true_cost.detach()
    # pairs where true[i] < true[j] (i should be cheaper than j)
    tc_row = tc.unsqueeze(1).expand(n, n)  # (n,n)
    tc_col = tc.unsqueeze(0).expand(n, n)  # (n,n)
    mask = (tc_row < tc_col)               # true where i cheaper than j

    pc_row = pred_cost.unsqueeze(1).expand(n, n)
    pc_col = pred_cost.unsqueeze(0).expand(n, n)
    # For correct ordering: pred[i] < pred[j], violation: pred[i] - pred[j] + margin > 0
    violation = F.relu(pc_row - pc_col + margin)
    loss = (violation * mask.float()).sum() / mask.float().sum().clamp(min=1.0)
    return loss
...
def compute_metrics(model, samples, layers):
    ...
    return {
        ...
        "per_sample":        sorted(per_sample, key=lambda r: -r[1])[:5],
    }
...
def train():
    ...
    print(f"  Top LMP outliers: {m8['per_sample'][:3]}")

    metrics = {
        "experiment": "improve_v7_rank_loss",
        "changes": [
            "pairwise ranking loss on pred_cost vs true_cost (W_RANK=0.2, margin=0.05)",
            "150 epochs from scratch (v5 converged at 120; v6 fine-tune overfit)",
            "same dc_frac decoder feature and W_AUX=0.5 as v5",
        ],
    ...
    print(f"  v5 (dc_frac): cost_gap=0.1235  lmp_mae=0.9993  (med 0.406)")
    print(f"  v7 (ranking): cost_gap={m8['cost_gap']['mean']:.4f}  lmp_mae={m8['lmp_mae']['mean']:.4f}  (med {m8['lmp_mae']['median']:.4f})")
