"""
Improvement experiment v8: DC bus voltage angles as 5th node feature + ranking loss.

Key changes vs v7:
1. DC bus voltage angles added as 5th node feature in node_feats (already in cache).
   - DC angle θ_i encodes nodal injection pattern: source buses have θ>0, sink buses θ<0.
   - Angle difference across a branch = DC flow / susceptance → directly encodes congestion.
   - Congested branches cause LMP divergence; this feature lets the model predict it.
2. Pairwise ranking loss (W_RANK=0.2) from v7 retained.
3. node_in: 4 → 5 to match new node_feats.
4. 150 epochs from scratch.

Hypothesis: DC bus angles tell the model WHICH branches are congested → can correctly
identify the marginal generator for each bus → better LMP prediction.

Target: reduce connecticut_16h LMP outlier (5.12 in v7) → LMP mean < 0.66 target.

Metrics: state/improve_v8_metrics.json
Checkpoint: state/checkpoints/design_a_v8.pt
"""
...
METRICS_OUT = "ralph/build/state/improve_v8_metrics.json"
CKPT_PATH   = "ralph/build/state/checkpoints/design_a_v8.pt"
...
class DesignAv8(nn.Module):
    """
    GNN encoder (node_in=5) + DC-dispatch-aware cost decoder.
    node_feats: [load_frac, gen_cap_frac, degree_norm, is_ref, dc_va_norm]
    cost decoder: [h_gen || gen_dc_frac]
    """
    def __init__(self, hidden_dim: int = 64, n_mp_layers: int = 3):
        super().__init__()
        D = hidden_dim
        self.encoder = MPGNNEncoder(node_in=5, edge_in=2, hidden_dim=D, n_layers=n_mp_layers)
...
def pairwise_rank_loss(pred_cost, true_cost, margin=0.05):
    ...
    tc = true_cost.detach()
    tc_row = tc.unsqueeze(1).expand(n, n)
    tc_col = tc.unsqueeze(0).expand(n, n)
    mask = tc_row < tc_col
...
def train():
    ...
    print(f"Node feats: {train_good[0].node_feats.shape}")
    ...
    model = DesignAv8(hidden_dim=HIDDEN_DIM, n_mp_layers=N_LAYERS)
    ...
    metrics = {
        "experiment": "improve_v8_dc_va_node_feat",
        "changes": [
            "DC bus voltage angle as 5th node feature (node_in=4→5)",
            "pairwise ranking loss W_RANK=0.2 (from v7)",
            "dc_frac decoder feature (from v5)",
            "150 epochs from scratch, W_AUX=0.5",
        ],
    ...
    print(f"  v8 (dc_va):   cost_gap={m8['cost_gap']['mean']:.4f}  lmp_mae={m8['lmp_mae']['mean']:.4f}")
